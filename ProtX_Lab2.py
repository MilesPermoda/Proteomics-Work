...
def setup(): # sets up the program for the 3 species
    ...
    if (count == 0):
        ...
        # OmicsDI allows a general query string.
        # We filter by 'omics_type:Proteomics' to exclude non-proteomics data
        global params
        params = {
...
def parser(data): # parses through the datasets and creates dataframes with relevant information
    ...
    for ds in data:
        # OmicsDI returns the source DB (e.g., 'pride', 'massive') and the ID
        accession = ds["id"]
        database = ds["source"]
        title = ds["title"]
